...
parser.add_argument(
    "--root_dir",
    type=str,
    default="path/to/your/h36m",
    help="Path to the h36m dataset directory."
)
parser.add_argument(
    "--output_dir",
    type=str,
...
# Human3.6M 里 32 关节 → 选取这 17 个常用关节
...
def process_cdf_to_npz(cdf_path, save_path):
    ...
    # 兼容现在 cdflib：cdf_info() 返回 dict，旧代码里当成对象用了 .zVariables
    info = cdf_data.cdf_info()
    if isinstance(info, dict):
        keys = info.get("zVariables", [])
    else:
        # 保险一点：如果不是 dict，就尝试用属性方式
        keys = getattr(info, "zVariables", [])

    if not keys:
        print(f"Warning: No variables found in {cdf_path}")
        return

    # 一般 Human3.6M 的 D2/D3_Positions.cdf 里只有一个 zVariable（比如 'Positional_Data'）
    pose_data = cdf_data.varget(keys[0])

    # 根据保存路径区分 3D / 2D（你原来的逻辑保留）
    if "3d" in save_path:
        # [num_frames, 96] → [num_frames, 32, 3]
        pose_data = pose_data.reshape(-1, 32, 3)
    else:
        # [num_frames, 64] → [num_frames, 32, 2]
