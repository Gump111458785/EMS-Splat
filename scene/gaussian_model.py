...
class GaussianModel:
    ...
    def get_scaling(self):
        """
        将 log-scale 激活后强制为各向同性：
        对每个高斯三轴尺度求均值，再 repeat 成 [N, 3]，保持梯度可导。
        """
        scaling = self.scaling_activation(self._scaling)           # [N, 3]
        mean_scale = scaling.mean(dim=1, keepdim=True)             # [N, 1]
        return mean_scale.repeat(1, 3)                             # [N, 3]
    ...
    def isotropic_loss(self, scaling: torch.Tensor = None, activated: bool = False):
        """
        极致各向同性惩罚：计算尺度在 dim=1 上的方差，并取所有高斯的均值。
        scaling: 可选输入张量，默认使用 self._scaling
        activated=False 表示输入仍在 log-scale，需要先 exp。
        """
        if scaling is None:
            scaling = self._scaling
        scaling_linear = self.scaling_activation(scaling) if not activated else scaling
        var_per_gaussian = scaling_linear.var(dim=1, unbiased=False)
        return var_per_gaussian.mean()
