...
class DataLoader:
    def __init__(
        ...
        self.n_views = nviews
        self.filtered_nviews = filtered_nviews if filtered_nviews is not None else nviews
        print(f"Number of views: {self.n_views}")

        if "h36m" in self.data_root or "dataset_fgy" in self.data_root:
    ...
    def _ensure_pose_channels(self, poses_2d):
        """
        Guarantee that every 2D pose sample carries (x, y, confidence) channels.
        If only x/y are present, synthesize an all-ones confidence map so downstream
        code can still apply per-joint weights consistently.
        """
        if poses_2d is None:
            return None
        if poses_2d.shape[-1] >= 3:
            return poses_2d[..., :3]
        if poses_2d.shape[-1] == 2:
            conf = np.ones((*poses_2d.shape[:-1], 1), dtype=poses_2d.dtype)
            return np.concatenate([poses_2d, conf], axis=-1)
        raise ValueError(f"Unexpected 2D pose shape: {poses_2d.shape}")
    ...
    def create_scene_mapping(self):
        ...
        for subject in subjects:
            ...
            if not os.path.isdir(subject_path_3d):
                continue
            if not os.path.isdir(subject_path_2d):
                print(f"Skipping subject {subject}: missing matching 2D directory {subject_path_2d}")
                continue
            ...
            for activity in activities:
                ...
                if not os.path.isdir(activity_path_3d):
                    continue
                if not os.path.isdir(activity_path_2d):
                    ...
                    continue
                if not os.path.isdir(gt_3d_path):
                    print(f"Missing GT path {gt_3d_path} for subject {subject}, activity {activity}. Skipping...")
                    continue
                ...
                if "panoptic" in self.data_root:
                    poses_3d_gt = self.load_npz(os.path.join(gt_3d_path, f"poses_filtered_{self.filtered_nviews}.npz"))

                if poses_3d_gt is None:
                    print(f"Missing GT poses for subject {subject}, activity {activity}. Skipping...")
                    continue
                ...
                if "gt" in self.initial_guess_dir:
                    ...
                    if poses_3d is None:
                        print(f"Missing initial guess poses for subject {subject}, activity {activity}. Skipping...")
                        continue

                if "panoptic" in self.data_root and "gt" not in self.initial_guess_dir:
                    poses_3d = np.array([poses_3d[i, ...] for i in range(0, poses_3d.shape[0], self.frame_step)])

                if poses_3d.shape[0] == 0 or poses_3d_gt.shape[0] == 0:
                    print(f"Empty pose arrays for subject {subject}, activity {activity}. Skipping...")
                    continue

                # Select nviews cameras
                cameras = self.cameras[:self.n_views]
                if len(cameras) != self.n_views:
                    raise ValueError(f"Requested {self.n_views} views but only got cameras={cameras}")
                ...
                for camera in cameras:
                    ...
                    poses_2d = self.load_npz(os.path.join(camera_path_2d, "poses.npz"))
                    poses_2d = self._ensure_pose_channels(poses_2d)
                    if "panoptic" in self.data_root:
                        poses_2d = self.load_npz(os.path.join(camera_path_2d, f"poses_filtered_{self.filtered_nviews}.npz"))
                        poses_2d = self._ensure_pose_channels(poses_2d)
                    if "gt" in self.poses_2d_dir or "panoptic" in self.data_root:
                        poses_2d = np.array([poses_2d[i, ...] for i in range(0, poses_2d.shape[0], self.frame_step)])
                        poses_2d = self._ensure_pose_channels(poses_2d)

                    if poses_2d is None:
                        raise FileNotFoundError(f"Missing 2D poses for subject {subject}, activity {activity}, camera {camera}")
                ...
                n_channels = poses_2d_fcam[0].shape[-1]
                poses_2d_fcam = np.stack(poses_2d_fcam, axis=0).reshape(self.n_views, -1, self.n_joints, n_channels)
                len_guess = poses_3d.shape[0]
                len_gt = poses_3d_gt.shape[0]
                len_2d = poses_2d_fcam.shape[1]
                if not (len_guess == len_gt == len_2d):
                    if "panoptic" in self.data_root:
                        raise ValueError(
                            "Panoptic data alignment mismatch for "
                            f"{subject}/{activity}: initial_guess={len_guess}, gt3d={len_gt}, poses2d={len_2d}. "
                            "Do not silently truncate; rebuild filtered GT/predictions/triangulation so all three lengths match."
                        )
                    safe_frames = min(len_guess, len_gt, len_2d)
                    print(
                        "H36M data alignment mismatch for "
                        f"{subject}/{activity}: initial_guess={len_guess}, gt3d={len_gt}, poses2d={len_2d}; "
                        f"using first {safe_frames} aligned frames."
                    )
                    poses_3d = poses_3d[:safe_frames, ...]
                    poses_3d_gt = poses_3d_gt[:safe_frames, ...]
                    poses_2d_fcam = poses_2d_fcam[:, :safe_frames, ...]
                else:
                    safe_frames = len_guess
                ...
                for frame in range(safe_frames):
                    ...
                    if scene_id >= self.start_id:
                        ...
                        # Keep 2D detections on CPU while building the full scene mapping.
                        # Moving every scene to CUDA here scales poorly for long Panoptic runs
                        # and can fail before training starts on restricted launch contexts.
                        poses_2d_scene = torch.tensor(poses_2d_scene)
                        camera_info_fcam = []
